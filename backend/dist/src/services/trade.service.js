...
export const TradeService = {
    ...
    async update(id, data) {
        if (data.entry && data.stop && data.target)
            data.rr = data.rr ?? calculateRR(data.entry, data.stop, data.target);
        if (data.entry && data.exit && data.contracts && data.direction)
            data.pnl = data.pnl ?? calculatePnL(data.entry, data.exit, data.contracts, data.direction);
        if (data.date)
            data.date = new Date(data.date);
        if (data.screenshotUrls)
            data.screenshotUrls = JSON.stringify(data.screenshotUrls);
    ...
    async getStats(account) {
        ...
        closed.forEach(t => {
            ...
            if (!byGrade[g])
                byGrade[g] = { wins: 0, total: 0, pnl: 0 };
            byGrade[g].total++;
        ...
        closed.forEach(t => {
            ...
            if (!byEmotion[e])
                byEmotion[e] = { wins: 0, total: 0, pnl: 0 };
            byEmotion[e].total++;
        ...
        closed.forEach(t => {
            const s = t.session;
            if (!bySession[s])
                bySession[s] = { wins: 0, total: 0, pnl: 0 };
            bySession[s].total++;
        ...
        closed.forEach(t => {
            ...
            if (!byModel[m])
                byModel[m] = { wins: 0, total: 0, pnl: 0 };
            byModel[m].total++;
        ...
        closed.forEach(t => {
            const d = t.direction;
            if (!byDirection[d])
                byDirection[d] = { wins: 0, total: 0, pnl: 0 };
            byDirection[d].total++;
        ...
        closed.forEach(t => {
            ...
            if (!daySessionMap[day])
                daySessionMap[day] = {};
            if (!daySessionMap[day][sess])
                daySessionMap[day][sess] = { wins: 0, total: 0 };
            daySessionMap[day][sess].total++;
            if ((t.pnl ?? 0) > 0)
                daySessionMap[day][sess].wins++;
        });
        ...
        closed.forEach(t => {
            ...
            if (!modelGradeMap[model])
                modelGradeMap[model] = {};
            if (!modelGradeMap[model][grade])
                modelGradeMap[model][grade] = { wins: 0, total: 0, pnl: 0 };
            modelGradeMap[model][grade].total++;
            modelGradeMap[model][grade].pnl += (t.pnl ?? 0);
            if ((t.pnl ?? 0) > 0)
                modelGradeMap[model][grade].wins++;
        });
        // Emotion Ã— Followed Plan insight
        const emotionPlanMap = {};
        trades.forEach(t => {
            const e = t.emotionalState;
            if (!emotionPlanMap[e])
                emotionPlanMap[e] = { followed: 0, notFollowed: 0, totalPnl: 0, trades: 0 };
            emotionPlanMap[e].trades++;
            emotionPlanMap[e].totalPnl += (t.pnl ?? 0);
            if (t.followedPlan)
                emotionPlanMap[e].followed++;
            else
                emotionPlanMap[e].notFollowed++;
        });
        return {
            ...
            // âœ… × ×›×•×Ÿ
            bestTrade: closed.reduce((b, t) => (t.pnl ?? -Infinity) > (b?.pnl ?? -Infinity) ? t : b, null),
            worstTrade: closed.reduce((w, t) => (t.pnl ?? Infinity) < (w?.pnl ?? Infinity) ? t : w, null),
            byGrade,
            byEmotion,
            bySession,
            byModel,
